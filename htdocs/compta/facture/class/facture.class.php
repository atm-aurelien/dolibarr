...
class Facture extends CommonInvoice {
	...
	// Id of bank account
	var $fk_bank;
	// Field to store bank id to use when payment mode is withdraw
	var $modelpdf;
	/**
	 * @deprecated
	 */
	...
	function insert_discount($idremise) {
		...
		if ($result > 0) {
			...
			$facligne->desc = $remise->description;
			// Description ligne
			$facligne->tva_tx = $remise->tva_tx;
			$facligne->subprice = -$remise->amount_ht;
			$facligne->fk_product = 0;
	...
	function list_replacable_invoices($socid = 0) {
		...
		$sql .= " AND f.paye = 0";
		// Pas classee payee completement
		$sql .= " AND pf.fk_paiement IS NULL";
		// Aucun paiement deja fait
		$sql .= " AND ff.fk_statut IS NULL";
		// Renvoi vrai si pas facture de remplacement
		if ($socid > 0)
		...
		if ($resql) {
			while ($obj = $this->db->fetch_object($resql)) {
				$return[$obj->rowid] = array('id' => $obj->rowid, 'ref' => $obj->facnumber, 'status' => $obj->fk_statut);
			}
			//print_r($return);
			return $return;
		} else {
	...
	function list_qualified_avoir_invoices($socid = 0) {
		...
		$sql .= " AND ff.type IS NULL";
		// Renvoi vrai si pas facture de remplacement
		$sql .= " AND f.type != " . self::TYPE_CREDIT_NOTE;
		// Type non 2 si facture non avoir
		if ($socid > 0)
			$sql .= " AND f.fk_soc = " . $socid;
		$sql .= " ORDER BY f.facnumber";
...
class FactureLigne extends CommonInvoiceLine {
	...
	function update($user = '', $notrigger = 0) {
		...
		if (empty($this->fk_parent_line))
			$this->fk_parent_line = 0;
		if (is_null($this->situation_percent))
