...
$permissiontoedit = $user->rights->facture->creer;
...
if (empty($reshook)) {
	...
	if ($conf->multidevises->enabled && $_REQUEST['currency']) {
		...
		$sql = "INSERT IGNORE INTO " . MAIN_DB_PREFIX . "document_currency (element_type, element_id, currency, rate) 
					VALUES ('invoice'," . $object->id . ",'" . $_REQUEST['currency'] . "'," . $line->current_rate . ")";
		$db->query($sql);
		$sql = "UPDATE " . MAIN_DB_PREFIX . "document_currency 
			SET currency='" . $_REQUEST['currency'] . "', rate=" . $line->current_rate . " 
			WHERE element_type='invoice' AND element_id=" . $object->id;
		$db->query($sql);
	...
	else if ($action == 'confirm_valid' && $confirm == 'yes' && ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->facture->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->facture->invoice_advance->validate)))) {
		$idwarehouse = GETPOST('idwarehouse');
	...
	else if ($action == 'confirm_modif' && ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->facture->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->facture->invoice_advance->unvalidate)))) {
		$idwarehouse = GETPOST('idwarehouse');
	...
	// Convertir en reduc
		...
		if ($object->type == Facture::TYPE_DEPOSIT && $object->paye == 1 && empty($discountcheck->id))
			$canconvert = 1;
		// we can convert deposit into discount if deposit is payed completely and not already converted (see real condition into condition used to show button converttoreduc)
	 ...
	 */
				...
				if ($origin && $originid) {
					...
					if ($object->origin == 'shipping') {
						...
						if (count($exp->linkedObjectsIds['commande']) > 0)
							$object->linked_objects['commande'] = $exp->linkedObjectsIds['commande'][0];
					}
					...
					if ($id > 0) {
						...
						if ($_POST['type'] == Facture::TYPE_DEPOSIT) {
								...
								if ($result > 0) {
									...
									for ($i = 0; $i < $numlines; $i++) {
										...
										if (empty($lines[$i]->qty))
											$qualified = 0;
										// We discard qty=0, it is an option
										if (!empty($lines[$i]->special_code))
											$qualified = 0;
										// We discard special_code (frais port, ecotaxe, option, ...)
										if ($qualified)
							...
							$result = $object->addline($langs->trans('Deposit'), $amountdeposit, // subprice
							1, // quantity
							...
							0, // info_bits
							'HT', 0, 0, // product_type
							1, $lines[$i]->special_code, $object->origin, 0, 0, 0, 0, $langs->trans('Deposit'));
						} else {
		...
		if ($id > 0 && !$error) {
			...
			if ($conf->multidevises->enabled) {
				...
				$line = $db->fetch_object($result);
				$sql = "INSERT IGNORE INTO " . MAIN_DB_PREFIX . "document_currency (element_type, element_id, currency, rate) 
								VALUES ('invoice',$id,'" . $_POST['currency'] . "'," . $line->current_rate . ")";
				$db->query($sql);
			}
	...
	// Add a new line
		...
		// Update line
		if (!$error) {
			$result = $object->updateline(GETPOST('lineid'), $description, $pu_ht, $qty, GETPOST('remise_percent'), $date_start, $date_end, $vat_rate, $localtax1_rate, $localtax2_rate, 'HT', $info_bits, $type, GETPOST('fk_parent_line'), 0, $fournprice, $buyingprice, $label, $special_code, $array_options, GETPOST('progress'), $_POST['units']);
...
if ($action == 'create') {
	...
	// Ref
	print '<tr><td class="fieldrequired">' . $langs->trans('Ref') . '</td><td colspan="3">' . $langs->trans('Draft') . '</td></tr>';
	...
	print '<tr><td valign="top" class="fieldrequired">' . $langs->trans('Type') . '</td><td colspan="3">';
	...
	// Payment term
	print '<tr><td class="nowrap">' . $langs->trans('PaymentConditionsShort') . '</td><td colspan="3">';
	...
	if (!empty($origin) && !empty($originid) && is_object($objectsrc)) {
		...
		print '<tr><td>' . $langs->trans($newclassname) . '</td><td colspan="3">' . $objectsrc->getNomUrl(1);
		...
		if ($conf->multidevises->enabled) {
			...
			print $form->selectCurrency($objectsrc->currency, "currency");
			print '<script>$("#socid").on("change", function(){var c=$(this).find("option[value=\""+$(this).val()+"\"]").attr("data-currency");$("#currency").val(c)});</script>';
			print '</td></tr>';
		}
		...
		if ($conf->multidevises->enabled) {
			print '<td class="nowrap right">' . price($objectsrc->total_ht_curr, '', $langs, 0, -1, -1, $objectsrc->currency) . '</td>';
			print '<td class="nowrap right">' . price($objectsrc->total_ht, '', $langs, 0, -1, -1, $conf->currency) . '</td>';

		} else {
			print '<td class="nowrap" colspan="3">' . price($objectsrc->total_ht, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
		}
		...
		if ($conf->multidevises->enabled) {
			...
			print '<td class="nowrap right">' . price($objectsrc->total_tva_curr, '', $langs, 0, -1, -1, $objectsrc->currency) . '</td>';
			print '<td class="nowrap right">' . price($objectsrc->total_tva, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
		} else {
			print '<td class="nowrap" colspan="3">' . price($objectsrc->total_tva, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
		}
		//print '<td class="nowrap" colspan="3">' . price($object -> total_tva, '', $langs, 0, -1, -1, $conf -> currency) . '</td>';
		print '</tr>';
		...
		if ($mysoc->localtax1_assuj == "1" || $object->total_localtax1 != 0)// Localtax1
		{
			...
			if ($conf->multidevises->enabled) {
				...
				print '<td class="nowrap right">' . price(round($objectsrc->total_localtax1 * $object->rate, 2), '', $langs, 0, -1, -1, $objectsrc->currency) . '</td>';
				print '<td class="nowrap right">' . price($objectsrc->total_localtax1, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
			} else {
				print '<td class="nowrap" colspan="3">' . price($objectsrc->total_localtax1, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
			}
			//print '<td class="nowrap" colspan="3">' . price($object -> total_localtax1, '', $langs, 0, -1, -1, $conf -> currency) . '</td>';
			print '<td></td></tr>';
		}
		if ($mysoc->localtax2_assuj == "1" || $object->total_localtax2 != 0)// Localtax2
		{
			...
			if ($conf->multidevises->enabled) {
				...
				print '<td class="nowrap right">' . price(round($objectsrc->total_localtax2 * $object->rate, 2), '', $langs, 0, -1, -1, $objectsrc->currency) . '</td>';
				print '<td class="nowrap right">' . price($objectsrc->total_localtax2, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
			} else {
				print '<td class="nowrap" colspan="3">' . price($objectsrc->total_localtax2, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
			}
			//print '<td class="nowrap" colspan="3">' . price($object -> total_localtax2, '', $langs, 0, -1, -1, $conf -> currency) . '</td>';
			print '</tr>';
		}
		...
		if ($conf->multidevises->enabled) {
			...
			print '<td class="nowrap right">' . price($objectsrc->total_ttc_curr, '', $langs, 0, -1, -1, $objectsrc->currency) . '</td>';
			print '<td class="nowrap right">' . price($objectsrc->total_ttc, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
		} else {
			print '<td class="nowrap" colspan="3">' . price($objectsrc->total_ttc, '', $langs, 0, -1, -1, $conf->currency) . '</td>';
		}
		//print '<td class="nowrap" colspan="3">' . price($object -> total_ttc, '', $langs, 0, -1, -1, $conf -> currency) . '</td>';
		print '</tr>';

		/*print '<tr><td>' . $langs -> trans('TotalHT') . '</td><td colspan="3">' . price($objectsrc -> total_ht) . '</td></tr>';
		 print '<tr><td>' . $langs -> trans('TotalVAT') . '</td><td colspan="3">' . price($objectsrc -> total_tva) . "</td></tr>";
		 if ($mysoc -> localtax1_assuj == "1" || $objectsrc -> total_localtax1 != 0)// Localtax1
		 {
		 print '<tr><td>' . $langs -> transcountry("AmountLT1", $mysoc -> country_code) . '</td><td colspan="3">' . price($objectsrc -> total_localtax1) . "</td></tr>";
		 }

		 if ($mysoc -> localtax2_assuj == "1" || $objectsrc -> total_localtax2 != 0)// Localtax2
		 {
		 print '<tr><td>' . $langs -> transcountry("AmountLT2", $mysoc -> country_code) . '</td><td colspan="3">' . price($objectsrc -> total_localtax2) . "</td></tr>";
		 }
		 print '<tr><td>' . $langs -> trans('TotalTTC') . '</td><td colspan="3">' . price($objectsrc -> total_ttc) . "</td></tr>";*/
	} else {
	...
	if ($absolute_creditnote > 0) {
		...
		if ($object->statut != 1 || $object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
			...
			$form->form_remise_dispo($_SERVER["PHP_SELF"] . '?facid=' . $object->id, 0, 'remise_id_for_payment', $soc->id, $absolute_creditnote, $filtercreditnote, 0);
	...
	print '<td rowspan="' . $nbrows . '" colspan="3" valign="top">';
	...
	if (!empty($conf->global->INVOICE_US_SITUATION)) {
		if ($object->type == 5 && ($object->situation_counter > 1)) {
			...
			print '</td>';
			print '<td align="right" colspan="3" class="nowrap">';
	...
	if ($conf->multidevises->enabled) {
		...
		if ($action != 'editcurrency')
			print '<a href="' . DOL_URL_ROOT . '/compta/facture.php?id=' . $object->id . '&action=editcurrency">' . img_edit() . '</a>';
		else
			print '&nbsp;';
		print '</td></tr></table>';
		print '<td class="nowrap">';
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline') {
		...
		if (empty($reshook)) {
			...
			if ($object->statut == 1) {
				...
				if ($resteapayer == $object->total_ttc && $object->paye == 0 && $ventilExportCompta == 0) {
					if (!$objectidnext && $object->is_last_in_cycle()) {
						if ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->facture->creer)) || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->invoice_advance->unvalidate))) {
			...
			if ((($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_REPLACEMENT) || ($object->type == Facture::TYPE_CREDIT_NOTE && empty($discount->id)) || ($object->type == Facture::TYPE_DEPOSIT && empty($discount->id))) && ($object->statut == 2 || $object->statut == 3) && $user->rights->facture->creer)// A paid invoice (partially or completely)
			{
	...
	if ($action != 'prerelance' && $action != 'presend') {
		...
		// List suggested for send to
		$formmail->withtocc = $liste;
		// List suggested for CC
