...
$usemargins = 0;
if (!empty($conf->margin->enabled) && !empty($object->element) && in_array($object->element, array('facture', 'propal', 'askpricesupplier', 'commande')))
	$usemargins = 1;
...
$colspan = 3;
// Col total ht + col edit + col delete
if (in_array($object->element, array('propal', 'askpricesupplier', 'facture', 'invoice', 'commande', 'order', 'order_supplier', 'invoice_supplier')))
	$colspan++;
// With this, there is a column move button
if ($conf->multidevises->enabled && $object->currency != $conf->currency)
	$colspan++;
...
<div id="add"></div><span class="hideonsmartphone"><?php echo $langs->trans('AddNewLine'); ?><
/span><?php // echo $langs->trans("FreeZone"); ?>
</td>
<?php if ($object->element == 'askpricesupplier') {
?>
<td align="right"><span id="title_fourn_ref"><?php echo $langs->trans('AskPriceSupplierRefFourn'); ?><
/span></td>
<?php } ?>
<td align="right"><span id="title_vat"><?php echo $langs->trans('VAT'); ?><
/span></td>
<td align="right"><span id="title_up_ht"><?php echo $langs->trans('PriceUHT'); ?><
/span></td>
<?php if($conf->multidevises->enabled && $object->currency!=$conf->currency) {
?>
<td align="right"><span id="title_up_ht"><?php echo $langs->trans('PriceUHT') . ' ' . $langs->getCurrencySymbol($object->currency); ?><
/span></td>
<?php } ?>
<?php if (! empty($inputalsopricewithtax)) {
?>
<td align="right"><span id="title_up_ttc"><?php echo $langs->trans('PriceUTTC'); ?><
/span></td>
<?php } ?>
<td align="right"><?php echo $langs->trans('Qty'); ?><
/td>
<?php
...
?>
<td align="right"><?php echo $langs->trans('ReductionShort'); ?><
/td>
<?php
...
if ($user->rights->margins->creer && !empty($conf->global->DISPLAY_MARGIN_RATES))
	echo '<td align="right" class="margininfos"><span class="np_marginRate">' . $langs->trans('MarginRate') . '</span></td>';
if ($user->rights->margins->creer && !empty($conf->global->DISPLAY_MARK_RATES))
	echo '<td align="right" class="margininfos"><span class="np_markRate">' . $langs->trans('MarkRate') . '</span></td>';
}
?>
...
if (!empty($conf->product->enabled) || !empty($conf->service->enabled)) {
	...
	if (empty($senderissupplier)) {
		...
		$ajaxoptions = array('update' => array('qty' => 'qty', 'remise_percent' => 'discount'), // html id tags that will be edited with which ajax json response key
...
$('#price_ht').val(Math.round(100*$('#price_ht_curr').val()/<?php echo $object->rate ?>)
/100);
})
$('#price_ht').on('keyup',function() {
$('#price_ht_curr').val(Math.round(100*$('#price_ht').val()*<?php echo $object->rate
?>
	)
	/100);
	})
...
?>v
ar main_max_dec_shown =<?php echo $conf->global->MAIN_MAX_DECIMALS_SHOWN; ?>
	;
	var main_rounding_unit = 
<?php echo $conf->global->MAIN_MAX_DECIMALS_UNIT; ?>
	;
...
<?php echo $conf->global->MAIN_MAX_DECIMALS_TOT; ?>
	...
	var decpart = '';
	if (pos >= 0)
		decpart = amount.substr(pos + 1).replace('/0+$/i', '');
	// Supprime les 0 de fin de partie decimale
	...
	// Replace of thousand before replace of dec to avoid pb if thousand is .
...
$.post('<?php echo DOL_URL_ROOT; ?>
	/fourn/ajax / getSupplierPrices.php
	', { 'idprod': $(this).val() }, function(data) {
	...
	var defaultbuyprice = '
<?php echo((isset($conf->global->MARGIN_TYPE) && $conf->global->MARGIN_TYPE == '1') ? 'bestsupplierprice' : ''); ?>
	';
	...
	var defaultbuyprice = '
<?php echo((isset($conf->global->MARGIN_TYPE) && $conf->global->MARGIN_TYPE == 'pmp') ? 'pmp' : ''); ?>
	';
...
<?php } ?>
	...
	function setforfree() {
		...
		jQuery("#idprod").val('');
		jQuery("#idprodfournprice").val('0');
		// Set cursor on not selected product
		...
		jQuery("#np_marginRate").show();
		// May no exists
		jQuery("#np_markRate").show();
		// May no exists
		jQuery(".np_marginRate").show();
		// May no exists
		jQuery(".np_markRate").show();
