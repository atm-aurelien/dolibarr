...
class pdf_muscadet extends ModelePDFSuppliersOrders
{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					if($conf->multidevises->enabled)
						$pdf->MultiCell($this->posxqty-$this->posxup-0.8, 3, price(round($object->lines[$i]->subprice*$object->rate,2)), 0, 'R', 0);
					else
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$price_ht=$object->total_ht + (! empty($object->remise)?$object->remise:0);
		if($conf->multidevises->enabled)
			$pdf->MultiCell($largcol2, $tab2_hl, price(round($price_ht*$object->rate,2)), 0, 'R', 1);
		else
			$pdf->MultiCell($largcol2, $tab2_hl, price($price_ht), 0, 'R', 1);
		...
		foreach( $this->tva as $tvakey => $tvaval )
		{
			if ($tvakey > 0)    // On affiche pas taux 0
			{
				...
				if($conf->multidevises->enabled)
					$pdf->MultiCell($largcol2, $tab2_hl, price(round($tvaval*$object->rate,2)), 0, 'R', 1);
				else
		...
		if (! $this->atleastoneratenotnull) // If no vat at all
		{
			...
			if (! empty($conf->global->FACTURE_LOCAL_TAX1_OPTION) && $conf->global->FACTURE_LOCAL_TAX1_OPTION=='localtax1on' && $object->total_localtax1>0)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if($conf->multidevises->enabled)
					$pdf->MultiCell($largcol2, $tab2_hl, price(round($object->total_localtax1*$object->rate,2)), 0, 'R', 1);
				else
			...
			if (! empty($conf->global->FACTURE_LOCAL_TAX2_OPTION) && $conf->global->FACTURE_LOCAL_TAX2_OPTION=='localtax2on' && $object->total_localtax2>0)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if($conf->multidevises->enabled)
					$pdf->MultiCell($largcol2, $tab2_hl, price(round($object->total_localtax2*$object->rate,2)), 0, 'R', 1);
				else
		...
		$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
		if($conf->multidevises->enabled)
			$pdf->MultiCell($largcol2, $tab2_hl, price(round($object->total_ttc*$object->rate,2)), 0, 'R', 1);
		else
	...
	function _tableau(&$pdf, &$object, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			$titre = $outputlangs->transnoentities("AmountInCurrency",$outputlangs->transnoentitiesnoconv("Currency".($object->currency!=$conf->currency ? $object->currency : $conf->currency)));
