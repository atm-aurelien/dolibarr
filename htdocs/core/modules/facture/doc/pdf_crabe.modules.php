...
class pdf_crabe extends ModelePDFFactures
{
	...
	function _tableau_versements(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($resql)
		{
			...
			while ($i < $num)
			{
				...
				if($conf->multidevises->enabled)
					$pdf->MultiCell(20, 3, price(round($obj->amount_ttc*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
				else
		...
		if ($resql)
		{
			...
			while ($i < $num) {
				...
				$pdf->SetXY($tab3_posx+21, $tab3_top+$y);
				if($conf->multidevises->enabled)
					$pdf->MultiCell(20, 3, price(round($sign * $row->amount*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
				else
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if($conf->multidevises->enabled)
			$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ht_curr + (! empty($object->remise)?$object->remise:0), 0, $outputlangs), 0, 'R', 1);
		else
					...
					foreach( $this->localtax1 as $localtax_type => $localtax_rate )
					{
						...
						foreach( $localtax_rate as $tvakey => $tvaval )
						{
							if ($tvakey!=0)    // On affiche pas taux 0
							{
								...
								if($conf->multidevises->enabled)
									$pdf->MultiCell($largcol2, $tab2_hl, price(round($object->rate*$tvaval,2), 0, $outputlangs), 0, 'R', 1);
								else
		...
		if ($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0)
		{
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			if($conf->multidevises->enabled)
				$pdf->MultiCell($largcol2, $tab2_hl, price(round($deja_regle + $depositsamount*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
			else
			...
			if ($creditnoteamount)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if($conf->multidevises->enabled)
					$pdf->MultiCell($largcol2, $tab2_hl, price(round($creditnoteamount*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
				else
			...
			if ($object->close_code == Facture::CLOSECODE_DISCOUNTVAT)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if($conf->multidevises->enabled)
					$pdf->MultiCell($largcol2, $tab2_hl, price(round(($object->total_ttc - $deja_regle - $creditnoteamount - $depositsamount)*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
				else
			...
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			if($conf->multidevises->enabled)
				$pdf->MultiCell($largcol2, $tab2_hl, price(round($resteapayer*$object->rate,2), 0, $outputlangs), 0, 'R', 1);
			else
