...
if (empty($reshook)) {
	...
	// Validation
	else if ($action == 'confirm_validate' && $confirm == 'yes' && ((empty($conf -> global -> MAIN_USE_ADVANCED_PERMS) && !empty($user -> rights -> propal -> creer)) || (!empty($conf -> global -> MAIN_USE_ADVANCED_PERMS) && !empty($user -> rights -> propal -> propal_advance -> validate)))) {
...
if ($action == 'create') {
	...
	if (!empty($conf -> global -> PROPAL_CLONE_ON_CREATE_PAGE) || !empty($conf -> global -> PRODUCT_SHOW_WHEN_CREATE))
		print '<br><table>';
	if (!empty($conf -> global -> PROPAL_CLONE_ON_CREATE_PAGE)) {
	...
	if (!empty($conf -> incoterm -> enabled)) {
		...
		if ($user -> rights -> propal -> creer)
			print '<a href="' . DOL_URL_ROOT . '/comm/propal.php?id=' . $object -> id . '&action=editincoterm">' . img_edit() . '</a>';
		else
			print '&nbsp;';
		print '</td></tr></table>';
		print '</td>';
	...
	if ($action != 'presend') {
		...
		if (empty($reshook)) {
			if ($action != 'statut' && $action != 'editline') {
				// Validate
				if ($object -> statut == Propal::STATUS_DRAFT && $object -> total_ttc >= 0 && count($object -> lines) > 0 && ((empty($conf -> global -> MAIN_USE_ADVANCED_PERMS) && !empty($user -> rights -> propal -> creer)) || (!empty($conf -> global -> MAIN_USE_ADVANCED_PERMS) && !empty($user -> rights -> propal -> propal_advance -> validate)))) {
